{{--
    Rank-change macro modal: generates /gpromote or /gdemote lines.
    Triggered by:
        $dispatch('open-rank-macro', { op: 'promote', ids: [memberId] })
        $dispatch('open-rank-macro', { op: 'demote', ids: [memberId] })

    Same shape as kick-macro-modal; the op flips the title, command, and
    button colour. Single character per click is the common path; multi
    is supported for batch promote/demote runs.
--}}
<div x-data="rankMacroModal()" x-cloak
     x-on:open-rank-macro.window="open($event.detail.op, $event.detail.ids)"
     x-show="visible"
    ...
    <div class="bg-panel border border-line rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto" @click.outside="close()">
        <header class="px-5 py-3 border-b border-line flex items-center justify-between">
            <h2 class="font-semibold text-ink">
                <span x-text="op === 'demote' ? 'Demote' : 'Promote'"></span>
...
function rankMacroModal() {
    ...
    return {
        ...
        async open(op, ids) {
            this.reset();
            this.op = op === 'demote' ? 'demote' : 'promote';
            this.visible = true;
            this.loading = true;
            try {
                const resp = await fetch('{{ route('roster.rank-macro.preview') }}', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json'},
                    body: JSON.stringify({op: this.op, member_ids: ids}),
                });
        ...
        // Mirror of RankMacroBuilder so deselecting in the UI doesn't
        // need a server round-trip. /gpromote and /gdemote use the
        // bare character name (no realm), same convention as /gremove.
        rebuild() {
            const limit = 255;
            const command = this.op === 'demote' ? '/gdemote' : '/gpromote';
            const selected = new Set(this.selectedIds);
            const lines = this.characters
                .filter(c => selected.has(c.id))
                .map(c => command + ' ' + c.char_name);
            const macros = [];
            let current = '';
        ...
        async confirm() {
            ...
            try {
                const resp = await fetch('{{ route('roster.rank-macro.confirm') }}', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json'},
                    body: JSON.stringify({op: this.op, member_ids: this.selectedIds, notes: this.notes}),
                });
                if (! resp.ok) throw new Error('HTTP ' + resp.status);
                this.close();
            } catch (e) {
