{{--
    Drop one of these somewhere on a page that has rows triggering it.
    Buttons elsewhere on the page open the modal by dispatching:
        $dispatch('open-kick-macro', { ids: [1, 2, 3] })

    The modal POSTs to /roster/kick-macro to fetch the cohort + macro,
    then rebuilds the macro client-side as the officer toggles the
    per-character checkboxes (mirrors KickMacroBuilder logic). Final
    "I've run it" hit POSTs to /roster/kick-macro/confirm to log the
    audit trail.
--}}
<div x-data="kickMacroModal()" x-cloak
     x-on:open-kick-macro.window="open($event.detail.ids)"
     x-show="visible"
            ...
            <template x-if="!loading && characters.length">
                <div>
                    <p class="text-xs text-muted">
                        Uncheck anyone you don't want to kick. The dashboard never kicks anyone -
                        copy the macro into WoW and run it there. Roster updates after the next GRM sync.
                    </p>
                    ...
                    <template x-if="skipped.length">
                        <div class="mt-3 text-xs text-amber-300/80">
                            <strong>Skipped (already gone):</strong>
...
function kickMacroModal() {
    ...
    return {
        ...
        async open(ids) {
            ...
            this.loading = true;
            try {
                const resp = await fetch('{{ route('roster.kick-macro.preview') }}', {
                    method: 'POST',
        ...
        // Mirror of KickMacroBuilder so deselecting in the UI doesn't
        // need a server round-trip. Char names are ASCII-only in WoW so
        ...
        async confirm() {
            ...
            try {
                const resp = await fetch('{{ route('roster.kick-macro.confirm') }}', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json'},
