{{--
    Unlink-from-alt-group macro modal. Triggered by:
        $dispatch('open-unlink-alt', { ids: [memberId] })

    Same shape as set-main-macro-modal but generates
    /run GRM.RemovePlayerFromAltGroup(...) lines.
--}}
<div x-data="unlinkAltMacroModal()" x-cloak
     x-on:open-unlink-alt.window="open($event.detail.ids)"
     x-show="visible"
            ...
            <template x-if="!loading && characters.length">
                <div>
                    <p class="text-xs text-muted">
                        Removes the selected character from its alt group in GRM. The dashboard
                        won't change groupings directly; copy the macro into WoW and run it.
                        The next GRM sync will reflect the unlinked state here.
                    </p>
...
function unlinkAltMacroModal() {
    ...
    return {
        ...
        async open(ids) {
            ...
            this.loading = true;
            try {
                const resp = await fetch('{{ route('roster.unlink-alt.preview') }}', {
                    method: 'POST',
        ...
        // Mirror of UnlinkAltMacroBuilder so deselecting in the UI
        // doesn't need a server round-trip. byteLength counts multi-
        // byte name chars correctly (Ñ takes 2 bytes etc.).
        rebuild() {
            const limit = 255;
            const selected = new Set(this.selectedIds);
            const lines = this.characters
                .filter(c => selected.has(c.id))
                .map(c => '/run GRM.RemovePlayerFromAltGroup("' + c.name.replace(/\\/g, '\\\\').replace(/"/g, '\\"') + '")');
            const macros = [];
            let current = '';
        ...
        async confirm() {
            ...
            try {
                const resp = await fetch('{{ route('roster.unlink-alt.confirm') }}', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json'},
